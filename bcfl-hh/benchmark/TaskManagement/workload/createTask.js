...
class StoreWorkload extends WorkloadModuleBase {
     ...
     */
    _generateWorkload(data) {

        let workload = [];
        let roundIndexes = [0,100,350,850,2850,3350, 5350];
        // var data = fs.readFileSync("./workload.json");
        // var data= JSON.parse(data);

        for(let i= 0; i < this.roundArguments.txnPerBatch; i++) {
            this.txIndex++;
            let _taskId = this.data[this.txIndex+roundIndexes[this.roundArguments.num-1]]._taskId ; 
            let _initialModel = this.data[this.txIndex+roundIndexes[this.roundArguments.num-1]]._initialModel ; 
            let _reward = this.data[this.txIndex+roundIndexes[this.roundArguments.num-1]]._reward; 
            let _totalRounds = this.data[this.txIndex+roundIndexes[this.roundArguments.num-1]]._totalRounds; 
            let sender = "0x25E85bF1af27415BeEFd0c1a4B3594A7569EbB23";
            let args = {
                contract :this.roundArguments.contract,
                verb: 'createTask',
                args: [_taskId, _initialModel, _reward, _totalRounds, sender],
                readOnly: false,
            }   
